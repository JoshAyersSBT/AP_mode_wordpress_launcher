...
def configure_hostapd():
    if not PASSWORD or len(PASSWORD) < 8 or len(PASSWORD) > 63:
        print(f"❌ Invalid WPA password: must be 8–63 characters. Got: {len(PASSWORD)}")
        exit(1)

    print("🛠️ Writing hostapd config...")
...
"""
    ...
    run(f'sed -i "s|^#DAEMON_CONF=.*|DAEMON_CONF=\\"{HOSTAPD_CONF}\\"|" {HOSTAPD_DEFAULT}', check=False)
    run(f'sed -i "s|^DAEMON_CONF=.*|DAEMON_CONF=\\"{HOSTAPD_CONF}\\"|" {HOSTAPD_DEFAULT}', check=False)
...
def start_services():
    ...
    run("dnsmasq --test")
    print("✅ dnsmasq config is valid.")

    print("🚀 Starting services...")
    run("systemctl restart hostapd")
    
    # Wait until uap0 exists before starting dnsmasq
    print("⏳ Waiting for uap0 to become available before dnsmasq...")
    for _ in range(10):
        if os.system("ip link show uap0 > /dev/null 2>&1") == 0:
            print("✅ uap0 interface is active.")
            break
        time.sleep(0.5)
    else:
        print("❌ uap0 not detected. Cannot start dnsmasq.")
        exit(1)

    print("🧪 Launching dnsmasq manually for uap0...")
    run(f"dnsmasq --no-daemon --interface={INTERFACE} "
        f"--bind-interfaces --dhcp-range=192.168.50.10,192.168.50.100,255.255.255.0,24h &", check=False)


def main():
    run("systemctl restart hostapd@uap0")
    run("systemctl restart dnsmasq@uap0")
