...
def load_config():
    ...
    # Use last 8 chars to keep SSID shorter but still identifiable per board
    SSID = f"BetaBox-{serial[-8:]}"
    log_info(f"Using autogenerated SSID: {SSID}")
...
def install_dnsmasq_uap0_override():
    """
    Install /usr/local/sbin/ensure_uap0.sh and a systemd override for dnsmasq
    so that, on every dnsmasq start, uap0 is ensured/created and the service
    is retried on failure.
    """
    log_info("Installing uap0 guard for dnsmasq (ensure_uap0.sh + override.conf)...")

    script_path = "/usr/local/sbin/ensure_uap0.sh"
    override_dir = "/etc/systemd/system/dnsmasq.service.d"
    override_path = os.path.join(override_dir, "override.conf")

    # Bash helper to ensure/repair uap0 before dnsmasq starts
    script_body = f"""#!/bin/bash
set -e

# Physical Wi-Fi interface and AP virtual interface
PHY_IF="${{PHY_IF:-{WLAN}}}"
AP_IF="${{AP_IF:-{INTERFACE}}}"
AP_ADDR="${{AP_ADDR:-{STATIC_AP_IP}/24}}"

echo "[ensure_uap0] Ensuring $AP_IF exists on $PHY_IF..."

# If AP_IF already exists, just bring it up and ensure it has the IP
if ip link show "$AP_IF" >/dev/null 2>&1; then
    echo "[ensure_uap0] $AP_IF already exists, bringing it up..."
    ip link set "$AP_IF" up || true

    if ! ip addr show dev "$AP_IF" | grep -q "$AP_ADDR"; then
        echo "[ensure_uap0] Assigning IP $AP_ADDR to $AP_IF..."
        ip addr add "$AP_ADDR" dev "$AP_IF" || true
    fi

    exit 0
fi

# Try to create the AP virtual interface using iw
if command -v iw >/dev/null 2>&1; then
    echo "[ensure_uap0] Creating $AP_IF from $PHY_IF..."
    iw dev "$PHY_IF" interface add "$AP_IF" type __ap || true
else
    echo "[ensure_uap0] ERROR: 'iw' command not found; cannot create $AP_IF."
    exit 1
fi

# Verify it exists now
if ! ip link show "$AP_IF" >/dev/null 2>&1; then
    echo "[ensure_uap0] ERROR: $AP_IF still does not exist after creation attempt."
    exit 1
fi

echo "[ensure_uap0] Bringing $AP_IF up and assigning IP..."
ip link set "$AP_IF" up || true
...
"""
    ...
    try:
        ...
        log_success(f"Wrote uap0 helper script: {script_path}")
    except Exception as e:
        log_warn(f"Could not write {script_path}: {e}")

    # Write systemd override
    try:
        os.makedirs(override_dir, exist_ok=True)
        override_body = f"""[Unit]
...
"""
        ...
        log_success(f"Wrote dnsmasq override: {override_path}")

        # Reload systemd so the override is recognized
        run("systemctl daemon-reload", check=False)
    except Exception as e:
        log_warn(f"Could not install dnsmasq override: {e}")
