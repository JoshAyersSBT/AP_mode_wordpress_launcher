...
for arg in "$@"; do
    case $arg in
        ...
        --config)
            while true; do
                OPTION=$(whiptail --title "PiPress Config Utility" --menu "Select an option:" 20 70 10                 "1" "Toggle USE_LOCAL (Currently: $USE_LOCAL)"                 "2" "Toggle FAST_LAUNCH (Currently: $FAST_LAUNCH)"                 "3" "Run install_hostapd_service.py"                 "4" "Run install_dnsmasq_service.py"                 "5" "Exit config utility" 3>&1 1>&2 2>&3)
